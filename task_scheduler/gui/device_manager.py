...
class AddVehicleDialog(QDialog):
    ...
    def init_ui(self):
        ...
        layout.addRow('设备ID:', self.id_input)
...
class DeviceManagerWidget(QWidget):
    ...
    def init_ui(self):
        ...
        remove_button = QPushButton('移除设备')
    ...
    def add_vehicle(self):
        ...
        if dialog.exec():
            try:
                ...
                self.refresh()
                QMessageBox.information(self, '成功', f'已添加设备: {vehicle.id}')
    ...
    def remove_vehicle(self):
        ...
        reply = QMessageBox.question(
            self, '确认',
            f'是否要移除设备 {vehicle_id}？\n注意：如果设备正在执行任务，移除可能会导致任务失败。',
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                self.refresh()
                QMessageBox.information(self, '成功', f'已移除设备: {vehicle_id}')
            except Exception as e:
                QMessageBox.warning(self, '错误', f'移除设备失败: {str(e)}')