...
- **定位系统**：
  ...
  - 支持历史轨迹记录与回放
- **通信系统**：
  - 5G USB Modem + 鼠洞协议（MAVLink over UDP）
  - WebSocket实时数据传输
  - 支持心跳检测、状态同步
  - 自动重连与错误恢复机制
...
- **左侧工具栏**：包含各类编辑工具
  - 选择工具：用于选择和编辑已有对象
  - 静态对象工具：用于创建障碍物、建筑、地标和站点
  - 区域工具：用于绘制限制区、交通区、天气区和自定义区域
  - 路径工具：用于规划地面路径、空中航线和混合路径
- **中央编辑区**：显示地图和编辑内容
  - 支持缩放和平移操作
  - 显示坐标网格
  - 实时预览绘制效果
...
- **选择对象**：使用选择工具点击对象
- **平移地图**：按住鼠标中键拖动
- **缩放地图**：使用鼠标滚轮或快捷键（Ctrl++ / Ctrl+-）

#### 2. 创建静态对象
1. 选择相应的静态对象工具（障碍物、建筑、地标或站点）
2. 在地图上按住左键并拖动，以设置对象位置和大小
3. 释放鼠标左键完成创建
4. 在右侧属性面板中设置对象属性

#### 3. 绘制区域
1. 选择相应的区域工具（限制区、交通区、天气区或自定义区域）
2. 在地图上点击设置区域顶点
3. 移动鼠标预览区域形状
4. 双击完成区域绘制
5. 右键取消绘制
